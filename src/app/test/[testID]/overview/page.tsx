...
const AttemptCard = ({ sub }: { sub: SubmissionStatusEntity }) => {
  ...
  const endDate = sub.submittedAt ?? "Waiting...";
  ...
  const rows = [
    ...
    { key: "start", value: <span className="text-black">{sub.createdAt}</span> },
    { key: "end", value: <span className="text-gray-500">{endDate}</span> },
  ];

  return (
    <div className="max-w-[50vh] flex-1 bg-gray-100 rounded-lg shadow-lg shadow p-4">
...
export default function TestPage() {
  ...
  const handleClick = async () => {
    if (userTest?.status === "continue") {
      ...
      await createSubmission(userTest?.userTestID as string);
      router.push(`/test/${testID}/attempt`);
      }
    }
  }
